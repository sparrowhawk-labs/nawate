<!-- jess:core:start -->
## jess — signed-URL state switching (demo/verification)

`jess` makes a signed URL jump straight to a specific app state (user type,
...
troubleshooting): **`docs/jess/README.md`**.

### Core API

- `SparrowhawkLabs\Jess\Facades\Jess::fragment(string $name, Closure $callback)`
  — register a named state fragment, typically from your own
  `AppServiceProvider::boot()`. The closure runs with jess's isolated demo
  SQLite connection already set as `database.default` — write ordinary
  Eloquent/Seeder code, no connection name needed.
- `Jess::link(array $fragments, string $redirectTo, int|string|null $userId = null): string`
...
3. **Jess is inert until `JESS_ENABLED=true`.** Set it only in
   local/staging/demo environments — never production.
4. **`config('jess.template_db_path')` must stay in sync with your
...
5. **If your host app's `SESSION_DRIVER` is `database`, switch it to
   `file`** (or any non-DB driver) in every environment where jess is
...
`php artisan jess:cleanup [--dry-run]` deletes demo sessions past their
