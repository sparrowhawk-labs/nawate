...
test('install creates AGENTS.md with the core section and CLAUDE.md importing it', function () {
    $this->artisan('nawate:install', ['--no-migrate' => true])->assertExitCode(0);

    $agents = file_get_contents(base_path('AGENTS.md'));
    expect($agents)->toContain('<!-- nawate:core:start -->');
    expect($agents)->toContain('Nawate::fragment');
    expect($agents)->toContain('<!-- nawate:core:end -->');
...
test('install is idempotent — running twice does not duplicate either file', function () {
    ...
    $agents = file_get_contents(base_path('AGENTS.md'));
    expect(substr_count($agents, '<!-- nawate:core:start -->'))->toBe(1);
...
test('--no-docs skips AGENTS.md and CLAUDE.md entirely', function () {
    $this->artisan('nawate:install', ['--no-migrate' => true, '--no-docs' => true])->assertExitCode(0);
