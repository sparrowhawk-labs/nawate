...
namespace SparrowhawkLabs\Nawate\Exceptions;
...
/**
 * Wraps whatever a fragment closure throws while running against nawate's
 * SQLite demo connection, so the failure reads as "fragment X broke, here's
 * why" instead of a bare PDO/QueryException trace with no nawate context.
 *
...
class FragmentExecutionException extends RuntimeException
{
    public static function forFragment(string $name, Throwable $previous): self
    {
        ...
        $message = "nawate fragment '{$name}' threw while running against the demo SQLite connection: "
            . $previous->getMessage();
    ...
    private static function likelyEngineIncompatibilityHint(Throwable $previous): ?string
    {
        ...
        return "This looks like a MySQL/PostgreSQL-specific SQL feature (function \"{$function}\") that "
            ...
            . 'procedure CALL. Nawate runs the demo environment on SQLite regardless of your production '
